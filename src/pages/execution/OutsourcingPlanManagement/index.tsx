...
const OutsourcingPlanManagement: React.FC = () => {
  ...
  const [searchParams, setSearchParams] = useState<{
    ...
    const saved = localStorage.getItem('outsourcingPlanManagementSearchParams');
    console.log("outsourcingPlanManagementSearchParams:" + saved);
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch {
        return { productType: ProductType.OUTSOURCED };
      }
    }
    return { productType: ProductType.OUTSOURCED };
  });
  ...
  useEffect(() => {
    localStorage.setItem('outsourcingPlanManagementSearchParams', JSON.stringify(searchParams));
  }, [searchParams]);
      ...
      <ProTable<PlanRuntime>
        ...
        toolBarRender={() => [
          <Button
              ...
              onClick={() => {
                // 清除所有查询条件
                localStorage.removeItem('outsourcingPlanManagementSearchParams');
                setSearchParams({ productType: ProductType.OUTSOURCED });
                // 清除表单中的输入值
